class JumpsizesController < ApplicationController
  ...
  def index
    if current_user.has_role? :admin
      #uncomment to see ALL jumpsizes
      #@jumpsizes = Jumpsize.search(params[:search]).paginate(:page => params[:page], :per_page => 100).order(sort_order('id'))
  ...
  private
    ...
    def jumpsize_params
      params.require(:jumpsize).permit(:size, :length1, :length2, :itemsperpage, :jumpsizecreator, :sortorder)
    end
end
